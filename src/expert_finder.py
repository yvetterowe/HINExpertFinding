...
class ExpertFinder(object):
    ...
    def infer(self):
		...
		self.dist_z_a = np.array([np.random.dirichlet(self.n_z_a[z]) for z in xrange(self.K)])
		self.dist_z_v = np.array([np.random.dirichlet(self.n_z_v[z]) for z in xrange(self.K)])
		if self.infer_dist_phrase:
			self.dist_z_p = np.array([np.random.dirichlet(self.n_z_p[z]) for z in xrange(self.K)])
...
if __name__ == "__main__":
	...
	doc2 = DocMeta(1, {3:2, 4:1, 0:1, 2:1}, [1,2], 1)
	dist_phrase = np.array([[0.4, 0.3, 0.2, 0.08, 0.02], [0.06, 0.03, 0.01, 0.5, 0.4]])
